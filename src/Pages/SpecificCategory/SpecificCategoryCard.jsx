      ...
      <div className="mt-auto grid grid-cols-2 gap-5">
        {!isDisabled ? (
          <Link
            disabled={applied}
            onClick={handleApply}
            className={`flex items-center justify-center bg-[#7566D9] py-3 text-gray-200 rounded-lg text-base lg:text-sm xl:text-lg ${applied && "bg-red-300 hover:bg-red-200 cursor-not-allowed"
              }`}
          >
            Apply Now
          </Link>
          ...
          <div
            className={`flex items-center justify-center bg-[#7566D9] py-3 text-gray-200 text-base lg:text-sm xl:text-lg rounded-lg ${isDisabled ? "cursor-not-allowed" : ""
              }`}
            onClick={(e) => {
              if (isDisabled) {
                e.preventDefault();
